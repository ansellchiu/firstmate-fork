...
fm_afk_launch_restore_backup() {  # <backup> <had-afk>
  ...
  rm -f "$FM_AFK_LAUNCH_STATE/.afk" \
    ...
    "$FM_AFK_LAUNCH_STATE/.subsuper-inject-wedged.count" || result=1
  if [ "$had_afk" -eq 1 ]; then
    cp "$backup/.afk" "$FM_AFK_LAUNCH_STATE/.afk" || result=1
  fi
  for artifact in .subsuper-escalations .subsuper-escalations.since .subsuper-inject-wedged .subsuper-inject-wedged.count; do
