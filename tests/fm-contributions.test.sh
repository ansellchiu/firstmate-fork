...
case "$fault" in concurrency)
  mkdir -p "$FORGE/inflight"
  : > "$FORGE/inflight/$$"
  sleep 0.5
  printf '%s\n' "$(ls "$FORGE/inflight" | wc -l | tr -d ' ')" >> "$FORGE/inflight-samples"
  sleep 0.5
  rm -f "$FORGE/inflight/$$" ;;
esac
case "$fault:$*" in
...
test_pull_observation_bounds_concurrent_forge_reads() { # one user token, at most three reads in flight
  local home out peak
  home=$(new_home concurrency)
  forge_home "$home"
  wrap_forge "$home"
  mutate_record "$home" delivery '.records[0].checked_at="2026-09-15T08:00:00Z"'
  printf 'concurrency\n' > "$home/forge/fault"
  out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" poll) || fail 'the bounded-concurrency poll failed'
  [ -z "$out" ] || fail "the bounded-concurrency poll woke: $out"
  jq -e --arg now "$NOW" '.records[0] | .checked_at == $now and .error == null' \
    "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'the bounded-concurrency poll did not observe the PR'
  peak=$(sort -n "$home/forge/inflight-samples" | tail -1)
  [ "$peak" -le 3 ] || fail "a pull observation ran $peak concurrent forge reads on one token"
  [ "$peak" -ge 2 ] || fail 'the independent pull reads stopped running in parallel'
  pass 'a pull observation keeps at most three forge reads in flight on one token'
}
