...
# hold <marker-env...> -> HOLDER_PID: a real non-platform process blocked on a
# fifo this test keeps open, whose environment is exactly the markers.
hold() {
...
hold_under() {
  ...
  mkfifo "$fifo"
  eval "exec ${HOLDER_FD}<>\"\$fifo\""
  ( export FM_HOLDER_BIN="$HOLDER_BIN" FM_HOLDER_KIND="$HOLDER_KIND" \
      FM_HOLDER_FIFO="$fifo" FM_HOLDER_PIDFILE="$pidfile"
    exec -a "$argv0" bash -c '
      if [ "$FM_HOLDER_KIND" = node ]; then
        env -i FM_HOLDER=1 "$FM_HOLDER_BIN" -e '\''require("fs").readFileSync(process.argv[1])'\'' "$FM_HOLDER_FIFO" &
      else
        env -i FM_HOLDER=1 "$FM_HOLDER_BIN" . "$FM_HOLDER_FIFO" &
      fi
      printf "%s\n" "$!" > "$FM_HOLDER_PIDFILE"
      wait
    ' "$@" ) &
...
# Prove the holder construction on this host: its environment must be readable,
# or every marker case would be vacuous.
...
case "$probe_env" in
  *FM_PROBE_MARKER=1*) ;;
  *) fail "this host does not expose the $HOLDER_KIND holder's environment ($HOLDER_BIN): $probe_env" ;;
esac
