...
A no-change heartbeat outcome explicitly reported with `task=fleet` and `silent=true` is delivered silently with no rendered note, while every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text; the store presents an unchanged routine fact about one task once per bounded window, so a repeat MAIN does not see was still stored and is still readable through `fm_branch_outcomes`.
