...
`tests/fm-pi-primary-live-e2e.test.sh` also proves the working ship replaces Pi's built-in working indicator while Calm is active on the credentialed provider path, and that the ship clears when the run settles, before continuing its ordinary watcher lifecycle.
On Pi >= 0.85 that built-in indicator is the composer's own titled top border rather than a `Working...` transcript row ([`verification/pi-composer-shapes.md`](verification/pi-composer-shapes.md)), so the guard requires that border absent from the generating Calm pane and, as its positive control, present on the following stock turn.
