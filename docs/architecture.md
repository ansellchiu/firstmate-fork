...
A declared external wait is bounded by the declaration itself rather than by the pane hash, so an idle pane whose rendered footer changes every poll stays on that bounded recheck cadence instead of re-escalating as an ordinary stale, while a live worker parked on a decision is still surfaced rather than silenced, and a wait that replaces another starts its own cadence.
...
An UNDECLARED stale notification firstmate has already handled and acknowledged does not re-present a byte-identical repeat for the same window within `FM_WAKE_REPEAT_SUPPRESS_SECS`, while a changed reason and every other wake kind still surface immediately; a declared wait is bounded by its own declaration instead, because the bare payload cannot tell one declaration from the next.
...
The snapshot also carries the portfolio block that `bin/fm-attention-lib.sh` classifies, so those views and intake read one captain-attention classification instead of each deriving its own; `--portfolio` emits that block alone for intake's cheap read.
...
A Pi primary conversation is also kept long-lived on purpose, so it is watched for the point where re-sending it stops being cheap; [docs/pi-primary-growth.md](pi-primary-growth.md) owns those measured warnings and the rotation that replaces the conversation without splitting a bounded action or dropping supervision.
...
Each layer keeps its own marker family for its own per-wake dedupe, but their two catch-all fleet scans cross-check: each scan starts from the furthest position either catch-all marker proves classified, and treats an unreadable log as already reported when either layer reported that same observed state.
Both layers stay alive across an away-mode transition, so without that cross-check a catch-all digest one layer presented and marked would be re-presented by the other on every scan, indefinitely, because the marker stores are the persistence and survive a restart.
...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr uses native agent-state submit confirmation on idle baselines, a composer empty fallback when native stays idle, and a pre-Enter idle-to-busy transition, read from the rendered busy footer or from native agent-state, when that baseline is unavailable.
Every queued-Enter decision is owned by `fm_composer_queued_enter_verdict` in `bin/fm-composer-lib.sh` - herdr asks it at both of its `pending + busy` sites, the early footer-transition one and the retries-exhausted one - while tmux and herdr provide only their backend-specific busy signals.
...
`data/projects.md` records each project's standing posture and optional `+yolo` merge flag as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; a ship spawn or validation trigger that is more outward-facing than the registered posture permits is refused, while a ship spawn that drops below the registered rigor prints a deviation notice and continues.
A posture that cannot be read at all - a mistyped `+annotation`, an unreadable registry - is likewise a refusal rather than a guess: the spawn, send, home seed, and fleet sync all surface the reader error and fail, with fleet sync reporting it as a `STUCK:` refusal and exiting non-zero.
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and `--list`, which feeds the portfolio attention classification.
...
Teardown is fail-closed for ship worktrees: dirty worktrees refuse, committed work must be landed before the worktree is returned, and a ship task whose typed completion receipt is missing or unreadable refuses in the same way.
Those receipts are the durable typed record of one completed outcome, written structurally by the scripts that already publish the outcome - PR registration, the proved merge, and cleanup - rather than composed at report time; [`bin/fm-receipt.sh`](../bin/fm-receipt.sh)'s header owns the schema, the anchor kinds, both stores, and the writer sequence.
...
`/updatefirstmate` checks the public canonical Firstmate repo before it concludes the private `origin` is current, then fast-forwards the running firstmate repo and registered secondmate homes from `origin` without touching project clones.
Public upstream is fetch-only: `origin` is the only push and pull-request target, and genuinely new public commits are reconciled on an isolated branch through a private pull request that lands before the fleet update runs.
...
The procedure and outcome vocabulary are owned by the [`/updatefirstmate` skill](../.agents/skills/updatefirstmate/SKILL.md); the relevant script headers own the mechanics; the upstream check, its degraded and refusal states, and the reconciliation mechanics are owned by `bin/fm-upstream.sh`, and the inventory that separates intentional private divergence from drift by [`private-divergence.md`](private-divergence.md).
